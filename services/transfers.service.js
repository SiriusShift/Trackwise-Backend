...
const validateTransfers = async (id) => {
  const expense = await prisma.transfer.findFirst({
    where: { id: parseInt(id) },
  });

  if (!expense) {
    throw new Error("Transfer not found"); // ❌ Throw error here
  }

  return expense;
};
const getTransfers = async (userId, query) => {
  const {
    search,
    pageIndex,
    pageSize,
  ...
  const filteredTransfers = transfers.filter((item) => item !== undefined);
  console.log(filteredTransfers, "filtered");
